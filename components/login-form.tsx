  ...
  return (
    <div className="flex flex-col">
        ...
        <div className="flex justify-center items-center gap-x-4 my-8">
            ...
            <span className="text-center flex-grow-1 text-sm text-muted-foreground">Or, login with your email</span>
            <span className="border border-t-1 w-[100px]"></span>
        </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-y-4 text-sm">
                <p>{`Don't have an account?`} <Link href="/signup" className="text-blue-500 hover:text-blue-400 font-semibold">Signup</Link></p>
                <Link href="/forgotpassword" className="text-blue-500 hover:text-blue-400 font-semibold">Forgot Password</Link>
            </div>
        ...
        <LoginFormLoader show={isLoading} />
        </Form>
    </div>
  )
}

function LoginFormLoader({ show } : { show: boolean }) {
  if (!show) return null
